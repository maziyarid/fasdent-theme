	...
	<section class="section section-bone">
		<div class="site-container content-split">
			...
			<div class="service-diagram" data-reveal>
				<?php foreach ( (array) $svc( 'diagram', array() ) as $index => $label ) : ?>
	...
	<section class="section section-bone bordered-section">
		<div class="site-container">
			...
			<div class="benefit-grid">
				<?php foreach ( (array) $svc( 'benefits', array() ) as $benefit ) : ?>
	...
	<section class="section section-dark">
		<div class="site-container">
			...
			<div class="steps-grid">
				<?php foreach ( (array) $svc( 'steps', array() ) as $step ) : ?><article class="step-card" data-reveal><div class="step-number"><?php echo esc_html( isset( $step[0] ) ? $step[0] : '' ); ?></div><h3><?php echo esc_html( isset( $step[1] ) ? $step[1] : '' ); ?></h3><p><?php echo $kses( isset( $step[2] ) ? $step[2] : '' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p></article><?php endforeach; ?>
			</div>
		</div>
	</section>
	...
	<section class="section section-bone bordered-section">
		<div class="narrow-container">
			...
			<div class="accordion-list">
				<?php foreach ( (array) $svc( 'faqs', array() ) as $index => $faq ) : $panel_id = 'service-faq-' . $alipasandi_service_key . '-' . $index; ?>
					<div class="accordion-item">
						<button class="accordion-trigger" type="button" aria-expanded="false" aria-controls="<?php echo esc_attr( $panel_id ); ?>" data-accordion-trigger><span><?php echo esc_html( isset( $faq[0] ) ? $faq[0] : '' ); ?></span><?php echo alipasandi_icon( 'chevron', 19 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></button>
						<div class="accordion-panel" id="<?php echo esc_attr( $panel_id ); ?>" hidden><?php echo $kses( isset( $faq[1] ) ? $faq[1] : '' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
					</div>
				<?php endforeach; ?>
			</div>
		</div>
