> **HISTORICAL / SUPERSEDED — NON-RUNTIME DOCUMENT**
> این فایل فقط سابقه handoff فاز قدیمی Figma است. هر Domain/NAP/Contact داخل آن تاریخی است و **نباید** به‌عنوان Production configuration استفاده شود. Runtime SSOT در Plugin → Settings → اطلاعات کلینیک است و URL سایت از `home_url('/')` می‌آید.
