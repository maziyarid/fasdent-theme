...
<footer class="site-footer">
	<div class="site-container footer-grid">
		<section class="footer-brand" aria-label="<?php esc_attr_e( 'درباره کلینیک', 'alipasandi-clinic' ); ?>">
			...
			<p>کلینیک دندانپزشکی تخصصی با رویکرد مدرن، دقیق و قابل اعتماد در تهران و نوشهر.</p>
		...
		<section class="footer-contact">
			...
			<p><?php echo alipasandi_icon( 'location', 18 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><span><?php echo esc_html( alipasandi_clinic_option( 'clinic_address' ) ); ?></span></p>
			<?php
			$address_2 = alipasandi_clinic_option( 'clinic_address_2' );
			$maps_2    = alipasandi_clinic_option( 'clinic_maps_2' );
			if ( $address_2 ) :
				?>
			<p>
				<?php echo alipasandi_icon( 'location', 18 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				<?php if ( $maps_2 ) : ?>
					<a href="<?php echo esc_url( $maps_2 ); ?>" target="_blank" rel="noopener noreferrer"><span><?php echo esc_html( $address_2 ); ?></span></a>
				<?php else : ?>
...
<aside class="contact-float" data-contact-float>
	...
	<button type="button" class="contact-toggle" aria-label="تماس با ما" aria-expanded="false" aria-controls="contact-options" data-contact-toggle>
		<span class="contact-badge" aria-hidden="true">1</span>
