...
function alipasandi_current_service_key() {
	...
	if ( function_exists( 'alipasandi_service_key_for_post' ) ) {
		return alipasandi_service_key_for_post( get_queried_object_id() );
	}
	$keys = function_exists( 'alipasandi_compat_service_keys' ) ? alipasandi_compat_service_keys() : array( 'implant', 'crown', 'surgery', 'general' );
	foreach ( $keys as $key ) {
		if ( is_page( $key ) ) { return $key; }
	}
	return '';
}
...
function alipasandi_output_schema() {
	...
	$graph = array();
	$clinic_complete = class_exists( 'Alipasandi_NAP_Helper' )
		? Alipasandi_NAP_Helper::is_complete()
		: '' !== trim( (string) alipasandi_clinic_option( 'clinic_business_name' ) )
			&& '' !== trim( (string) alipasandi_clinic_option( 'clinic_street' ) )
			&& '' !== trim( (string) alipasandi_clinic_option( 'clinic_city' ) )
			&& '' !== trim( (string) alipasandi_clinic_option( 'clinic_region' ) )
			&& 1 === preg_match( '/^[A-Za-z]{2}$/', trim( (string) alipasandi_clinic_option( 'clinic_country' ) ) )
			&& 1 === preg_match( '/^\+[1-9][0-9]{7,14}$/', trim( (string) alipasandi_phone_href() ) );
	if ( $clinic_complete ) {
		$address = class_exists( 'Alipasandi_NAP_Helper' )
			? Alipasandi_NAP_Helper::structured_address()
			: array(
				'@type'           => 'PostalAddress',
				'streetAddress'   => trim( (string) alipasandi_clinic_option( 'clinic_street' ) ),
				'addressLocality' => trim( (string) alipasandi_clinic_option( 'clinic_city' ) ),
				'addressRegion'   => trim( (string) alipasandi_clinic_option( 'clinic_region' ) ),
				'addressCountry'  => strtoupper( trim( (string) alipasandi_clinic_option( 'clinic_country' ) ) ),
			);
		...
		$clinic = array(
			...
			'url'       => $site_url,
			'image'     => alipasandi_open_graph_image(),
			'telephone' => trim( (string) alipasandi_phone_href() ),
			'address'   => $address,
		);
		$map = trim( (string) alipasandi_clinic_option( 'clinic_maps' ) );
		if ( '' !== $map ) { $clinic['hasMap'] = $map; }
		...
		if ( '' !== $lat && '' !== $lng && is_numeric( $lat ) && is_numeric( $lng ) && (float) $lat >= -90 && (float) $lat <= 90 && (float) $lng >= -180 && (float) $lng <= 180 ) {
			$clinic['geo'] = array( '@type'=>'GeoCoordinates', 'latitude'=>(float)$lat, 'longitude'=>(float)$lng );
		}
		if ( function_exists( 'alipasandi_parse_opening_hours' ) ) {
			...
			if ( empty( $hours['errors'] ) && ! empty( $hours['specs'] ) ) { $clinic['openingHoursSpecification'] = $hours['specs']; }
		}
		$graph[] = $clinic;
	}
	...
	if ( $service_key ) {
		...
		if ( $clinic_complete ) { $service_schema['provider'] = array( '@id' => $clinic_id ); }
	...
	if ( is_singular( 'post' ) ) {
		...
		if ( $clinic_complete ) { $article_schema['publisher'] = array( '@id' => $clinic_id ); }
