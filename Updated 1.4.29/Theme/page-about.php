...
<main id="main-content">
	<section class="inner-hero">
		<div class="site-container inner-hero-grid">
			<div class="inner-hero-copy" data-reveal>
				...
				<a class="button button-gold" href="<?php echo esc_url( alipasandi_page_url( 'appointments' ) ); ?>"><?php echo alipasandi_icon( 'calendar', 17 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> درخواست نوبت</a>
			</div>
	...
	<section class="section-compact section-bone trust-strip"><div class="site-container stats-grid <?php echo alipasandi_show_treatment_count() ? 'stats-grid--4' : 'stats-grid--3'; ?>">
		<?php if ( alipasandi_show_treatment_count() ) : ?><div class="stat-item"><span class="stat-icon" aria-hidden="true"><?php echo alipasandi_icon( 'check', 24 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span><strong class="stat-number">+۱۰٬۰۰۰</strong><span class="stat-label">کیس درمانی</span><span class="stat-note">تجربه انجام درمان‌های دندانپزشکی</span></div><?php endif; ?>
		<div class="stat-item"><span class="stat-icon" aria-hidden="true"><?php echo alipasandi_icon( 'shield', 24 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span><strong class="stat-number">دقت</strong><span class="stat-label">در تشخیص</span><span class="stat-note">ارزیابی پیش از انتخاب درمان</span></div>
		<div class="stat-item"><span class="stat-icon" aria-hidden="true"><?php echo alipasandi_icon( 'clipboard', 24 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span><strong class="stat-number">شفافیت</strong><span class="stat-label">در ارتباط</span><span class="stat-note">توضیح گزینه‌ها و محدودیت‌ها</span></div>
		<div class="stat-item"><span class="stat-icon" aria-hidden="true"><?php echo alipasandi_icon( 'check', 24 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span><strong class="stat-number">پیگیری</strong><span class="stat-label">پس از درمان</span><span class="stat-note">متناسب با نیاز هر بیمار</span></div>
	</div></section>

	<section class="section-compact section-dark"><div class="site-container final-cta"><h2>برای آشنایی و مشاوره آماده‌اید؟</h2><p>برای ارزیابی اولیه و بررسی شرایط درمان، با کلینیک در تماس باشید.</p><div class="button-row"><a class="button button-gold" href="<?php echo esc_url( alipasandi_page_url( 'appointments' ) ); ?>">درخواست وقت مشاوره</a><?php if ( alipasandi_phone_href() ) : ?><a class="phone-link" href="tel:<?php echo esc_attr( alipasandi_phone_href() ); ?>"><?php echo alipasandi_icon( 'phone', 18 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><span dir="ltr"><?php echo esc_html( alipasandi_clinic_option( 'clinic_phone' ) ); ?></span></a><?php endif; ?></div></div></section>
</main>
<?php get_footer(); ?>
