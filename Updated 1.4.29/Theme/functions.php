...
function alipasandi_companion_plugin_admin_notice() {
	...
	$message = 'missing' === $state
		...
		: 'نسخه افزونه Alipasandi Service Content قدیمی است. حداقل نسخه 1.3.4 لازم است؛ فرم‌ها و مسیرهای Write تا ارتقا غیرفعال هستند.';
...
if ( ! function_exists( 'alipasandi_phone_href' ) ) {
	function alipasandi_phone_href() {
		...
		$phone = alipasandi_normalize_digits( $phone );
		$phone = preg_replace( '/[\s().-]+/u', '', $phone );
		$phone = preg_replace( '/[\x{200E}\x{200F}\x{202A}-\x{202E}\x{2066}-\x{2069}]/u', '', (string) $phone );
		return 1 === preg_match( '/^\+[1-9][0-9]{7,14}$/', (string) $phone ) ? $phone : '';
	}
}
...
function alipasandi_brand_logo( $extra_class = '' ) {
	...
	$brand_name = trim( (string) alipasandi_clinic_option( 'clinic_business_name' ) );
	if ( '' === $brand_name ) {
		$brand_name = get_bloginfo( 'name' );
	}
	?>
	<a class="<?php echo esc_attr( $classes ); ?>" href="<?php echo esc_url( home_url( '/' ) ); ?>" aria-label="<?php echo esc_attr( sprintf( '%s — صفحه اصلی', $brand_name ) ); ?>">
		...
		<span class="brand-copy"><strong><?php echo esc_html( $brand_name ); ?></strong><small>DENTAL CLINIC</small></span>
	</a>
	<?php
}

/**
 * Public service links from the plugin-owned registry.
 *
 * The fallback is presentation-only compatibility for a missing/outdated
 * companion plugin. Production navigation, appointment validation and service
 * content all use the same normalized registry when Plugin 1.3.4 is ready.
 */
function alipasandi_service_links() {
	$fallback = array(
		array( 'key'=>'implant', 'label'=>'ایمپلنت دندان', 'slug'=>'implant', 'page_slug'=>'services/implant', 'icon'=>'implant' ),
		array( 'key'=>'crown', 'label'=>'روکش دندان', 'slug'=>'crown', 'page_slug'=>'services/crown', 'icon'=>'crown' ),
		array( 'key'=>'surgery', 'label'=>'جراحی و لثه', 'slug'=>'surgery', 'page_slug'=>'services/surgery', 'icon'=>'surgery' ),
		array( 'key'=>'general', 'label'=>'درمان عمومی', 'slug'=>'general', 'page_slug'=>'services/general', 'icon'=>'general' ),
	);
	$items = array();
	if ( alipasandi_service_plugin_ready() && function_exists( 'alipasandi_service_registry' ) ) {
		foreach ( alipasandi_service_registry() as $item ) {
			...
			$items[] = array(
				'key'       => $item['key'],
				'label'     => $item['label'],
				'slug'      => wp_basename( $item['page_slug'] ),
	...
	foreach ( $items as &$item ) {
		$item['url'] = alipasandi_page_url( $item['page_slug'] );
	}
	unset( $item );
	return $items;
}

/** Return whether the queried page owns the given stable service key. */
function alipasandi_is_current_service( $key ) {
	$key = sanitize_key( $key );
	if ( ! is_page() ) {
		return false;
	}
	if ( function_exists( 'alipasandi_service_key_for_post' ) ) {
		return $key === alipasandi_service_key_for_post( get_queried_object_id() );
	}
	return is_page( $key );
}
...
function alipasandi_primary_links() {
	$service_active = is_page( 'services' );
	foreach ( alipasandi_service_links() as $service_link ) {
		$service_active = $service_active || alipasandi_is_current_service( $service_link['key'] );
	}
	return array(
		...
		array( 'label' => 'خدمات', 'url' => alipasandi_page_url( 'services' ), 'active' => $service_active, 'current' => is_page( 'services' ), 'children' => alipasandi_service_links() ),
		...
		if ( $mobile ) {
			...
			if ( $has_children ) {
				...
				foreach ( $link['children'] as $child ) {
					$child_current = alipasandi_is_current_service( $child['key'] );
