...
| حوزه | Owner |
|---|---|
| Presentation/Layout | Theme 1.4.29 |
| Service storage/editor/migration/revisions/export/health | Plugin 1.3.4 |
| NAP settings + Rank Math NAP sync | Plugin 1.3.4 |
| Contact/Appointment validation, rate limit, mail request | Plugin 1.3.4 |
...
Site URL Option ندارد؛ **`home_url('/')` SSOT** است. Phone/Address/Email/Map/Social و City/Region/Country هیچ Production default عملیاتی ندارند و باید فقط از مقادیر رسمیِ تأییدشده Clinic configure شوند. `clinic_address` متن کامل نمایشی و `clinic_street` جزء ساختاری خیابان است؛ هر تغییر Location باید هر دو مسیر را synchronized نگه دارد.

## 3) Opening hours / appointment request

Grammar کامل در `OPENING-HOURS-CONTRACT-FA.md`. Booking slots سراسری هر خط `HH:MM` هستند و `MO=09:00,10:30` یا `FR=CLOSED` override روزانه می‌دهد. انتخاب کاربر **درخواست زمان پیشنهادی** است، نه Availability لحظه‌ای یا رزرو قطعی. Horizon و Lead باید صریحاً توسط Owner ذخیره و تأیید شوند؛ default helper به‌تنهایی Form readiness را PASS نمی‌کند. Theme UI و Server validator همان policy، `wp_timezone()` و closing end-exclusive را مصرف می‌کنند.

Timezone عملیاتی همان WordPress timezone تأییدشده توسط Owner است؛ معیار کد `wp_timezone()`/`wp_timezone_string()` است، نه PHP/server timezone.
...
`SECURITY-NOTES-1.4.26-FA.md` سابقه تاریخی است؛ Runbook 1.4.29/1.3.4 مرجع Release جاری است. Raw IP ذخیره نمی‌شود؛ limiter از HMAC ناشناس استفاده می‌کند. Proxy header فقط بعد از allowlist Hosting trust شود. Abuse owner در scale = Edge/WAF.

## 10) Updates / rollback

Production pairing این Release فقط Theme 1.4.29 + Plugin 1.3.4 با Hashهای Manifest بیرونی است. `RELEASE-ROLLBACK-CONTRACT-1.4.26-FA.md` صرفاً Historical است. قبل update: DB + uploads backup، restore rehearsal، Service JSON export، artifact hash verification و Rank Math config backup. Update مستقیم Production بدون Staging ممنوع.
