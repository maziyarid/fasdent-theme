(function () {
  ...
  if (bookingForm) {
    ...
    const timeInputs = Array.from(bookingForm.querySelectorAll('input[name="time"][data-days]'));
    let currentStep = 1;

    const dayCodeForDate = (value) => {
      const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(value || "");
      if (!match) return "";
      const codes = ["SU", "MO", "TU", "WE", "TH", "FR", "SA"];
      return codes[new Date(Date.UTC(Number(match[1]), Number(match[2]) - 1, Number(match[3]))).getUTCDay()];
    };

    const filterTimesForDate = () => {
      const day = dayCodeForDate(dateField ? dateField.value : "");
      timeInputs.forEach((input) => {
        const allowed = input.dataset.days.split(",").filter(Boolean);
        const available = !day || allowed.includes(day);
        const wrapper = input.closest("[data-time-choice]");
    ...
    if (dateField) dateField.addEventListener("change", filterTimesForDate);
    ...
    bookingForm.addEventListener("submit", (event) => {
      // Validate the whole form, not only the current step panel.
      if (!isFormComplete()) {
        event.preventDefault();
        bookingForm.classList.add("show-validation");
        // Jump to the first incomplete step so the user sees what is missing.
      ...
      if (submit) {
        // Do NOT disable the submit control inside the submit handler — that can
        // cancel the navigation/request in some browsers. Visual-only loading state.
        submit.classList.add("is-loading");
        submit.setAttribute("aria-disabled", "true");
        const label = submit.querySelector("span") || submit;
        if (submit.childElementCount) {
          // Keep structure; append status for screen readers.
          submit.setAttribute("data-original-html", submit.innerHTML);
          submit.innerHTML = "در حال ارسال…";
        } else {
          submit.textContent = "در حال ارسال…";
        }
      }
      // Allow the browser to continue the normal POST to admin-post.php.
    });
