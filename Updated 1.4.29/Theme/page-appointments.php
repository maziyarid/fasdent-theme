...
$interoperability = $form_ready && function_exists( 'alipasandi_validate_booking_hours_interoperability' ) ? alipasandi_validate_booking_hours_interoperability() : array();
$time_days = array();
foreach ( isset( $interoperability['usable'] ) && is_array( $interoperability['usable'] ) ? $interoperability['usable'] : array() as $day => $day_times ) {
	foreach ( $day_times as $day_time ) { $time_days[ $day_time ][] = $day; }
}
if ( $form_ready ) {
	$times = array_keys( $time_days );
...
<main id="main-content">
	<section class="inner-hero">
		<div class="narrow-container section-heading" style="padding-block:64px;position:relative">
			<span class="eyebrow">نوبت‌دهی آنلاین</span><h1>درخواست نوبت</h1><p>درخواست برای مراجعه<?php echo '' !== $city ? ' در <strong>' . esc_html( $city ) . '</strong>' : ''; ?> ثبت می‌شود. تاریخ و ساعت انتخاب‌شده پیشنهادی است و نوبت پس از تماس کلینیک قطعی خواهد شد.</p>
		</div>
	</section>
	...
	<section class="section section-bone">
		<div class="narrow-container" id="clinic-form">
			...
			<?php elseif ( 'rate_limited' === $status ) : ?>
				<div class="form-message error" role="alert" tabindex="-1" autofocus>تعداد درخواست‌ها بیش از حد مجاز است. لطفاً ۱۵ دقیقه دیگر تلاش کنید<?php echo $phone_href ? ' یا با کلینیک تماس بگیرید' : ''; ?>.</div>
			<?php elseif ( in_array( $status, array( 'error', 'invalid', 'mail_error', 'configuration_error', 'unavailable' ), true ) ) : ?>
				<div class="form-message error" role="alert">ثبت درخواست کامل نشد. لطفاً اطلاعات را بررسی و دوباره تلاش کنید<?php echo $phone_href ? ' یا با کلینیک تماس بگیرید' : ''; ?>.</div>
			<?php endif; ?>

			<?php if ( '' !== $city || '' !== $address ) : ?><div class="booking-location-note" role="note">
				<strong>محل مراجعه<?php echo '' !== $city ? ': ' . esc_html( $city ) : ''; ?></strong>
				...
				<div class="booking-shell">
					...
					<form class="booking-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post" data-booking-form novalidate>
						...
						<section class="booking-step" data-booking-step="2" hidden>
							...
							<div class="time-grid">
								<?php foreach ( $times as $index => $time ) : $id = 'time-choice-' . $index; $days = isset( $time_days[ $time ] ) ? implode( ',', $time_days[ $time ] ) : ''; ?>
									<div data-time-choice><input class="choice-input" id="<?php echo esc_attr( $id ); ?>" type="radio" name="time" value="<?php echo esc_attr( $time ); ?>" data-days="<?php echo esc_attr( $days ); ?>"><label class="choice-label" for="<?php echo esc_attr( $id ); ?>"><?php echo esc_html( $time ); ?></label></div>
								<?php endforeach; ?>
							</div>
						</section>
			...
			<?php else : ?>
				<div class="booking-shell">
					<div class="form-message error" role="status" aria-live="polite" aria-atomic="true"><strong>سامانه ثبت درخواست نوبت موقتاً در دسترس نیست.</strong><?php if ( $phone_href ) : ?><br>برای هماهنگی، لطفاً با کلینیک تماس بگیرید.<?php else : ?><br>تنظیمات ارتباطی و برنامه نوبت‌دهی باید توسط مدیر سایت تکمیل شود.<?php endif; ?></div>
				</div>
			<?php endif; ?>
