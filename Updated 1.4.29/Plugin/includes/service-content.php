...
/** Return service content without the schema marker for no-content-change proofs. */
function alipasandi_service_content_payload( $meta ) {
	if ( ! is_array( $meta ) ) {
		return array();
	}
	unset( $meta['schema_version'] );
	return function_exists( 'alipasandi_canonicalize_snapshot' ) ? alipasandi_canonicalize_snapshot( $meta ) : $meta;
}

/** Stable SHA-256 of service content, explicitly excluding schema_version. */
function alipasandi_service_content_sha256( $meta ) {
	return hash( 'sha256', wp_json_encode( alipasandi_service_content_payload( $meta ), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) );
}

/** Human-readable evidence inventory for a service record. */
function alipasandi_service_content_evidence( $meta ) {
	$meta = is_array( $meta ) ? $meta : array();
	return array(
		'h1'             => trim( (string) ( $meta['title'] ?? '' ) ) . trim( (string) ( $meta['title_gold'] ?? '' ) ),
		'intro_present'  => '' !== trim( wp_strip_all_tags( (string) ( $meta['intro'] ?? '' ) ) ),
		'faq_count'      => is_array( $meta['faqs'] ?? null ) ? count( $meta['faqs'] ) : 0,
		'cta_present'    => '' !== trim( (string) ( $meta['cta_title'] ?? '' ) ),
		'image_present'  => ! empty( $meta['image_id'] ) || ! empty( $meta['image'] ),
		'internal_links' => preg_match_all( '/<a\b[^>]*href=/i', maybe_serialize( $meta ) ),
	);
}

/** Diagnose schema-marker and content safety independently. */
function alipasandi_service_schema_diagnosis( $page_id ) {
	if ( ! alipasandi_service_meta_exists( $page_id ) ) {
		return array( 'categories'=>array( 'malformed_meta' ), 'repairable'=>false, 'reason'=>'meta_absent' );
	}
	$all_values = get_post_meta( $page_id, ALIPASANDI_SERVICE_META_KEY, false );
	if ( 1 !== count( $all_values ) ) {
		return array( 'categories'=>array( 'malformed_meta' ), 'repairable'=>false, 'reason'=>'duplicate_meta_rows' );
	}
	$meta = get_post_meta( $page_id, ALIPASANDI_SERVICE_META_KEY, true );
	if ( ! is_array( $meta ) ) {
		return array( 'categories'=>array( 'malformed_meta' ), 'repairable'=>false, 'reason'=>'meta_not_array' );
	}
	$categories = array();
	if ( ! array_key_exists( 'schema_version', $meta ) ) {
		$categories[] = 'missing_schema_marker';
	} elseif ( ALIPASANDI_SERVICE_META_SCHEMA !== (int) $meta['schema_version'] ) {
		$categories[] = 'unknown_schema_version';
	}
	if ( empty( $meta['title'] ) || empty( $meta['intro'] ) ) {
		$categories[] = 'required_field_content_missing';
	}
	$with_current_marker = $meta;
	$with_current_marker['schema_version'] = ALIPASANDI_SERVICE_META_SCHEMA;
	$sanitized = alipasandi_sanitize_service_meta( $with_current_marker );
	if ( alipasandi_service_content_sha256( $with_current_marker ) !== alipasandi_service_content_sha256( $sanitized ) ) {
		$categories[] = 'sanitizer_drift';
	}
	$categories = array_values( array_unique( $categories ) );
	// A missing marker is safe to add when the payload already matches the
	// current sanitizer. An unknown marker may represent a future/different
	// schema and must never be silently rewritten to the current version.
	$repairable = array( 'missing_schema_marker' ) === $categories;
	return array(
		'categories' => $categories,
		'repairable' => $repairable,
		'evidence'   => alipasandi_service_content_evidence( $meta ),
	);
}

/** Build an immutable dry-run plan; only a schema marker may ever change. */
function alipasandi_service_schema_repair_plan() {
	$plan = array(
		'schema_version' => ALIPASANDI_SERVICE_META_SCHEMA,
	...
	foreach ( alipasandi_service_keys() as $key ) {
		$page = alipasandi_get_service_page( $key );
		if ( ! $page ) {
			$plan['services'][ $key ] = array( 'page_id'=>0, 'action'=>'blocked', 'diagnosis'=>array( 'categories'=>array( 'malformed_meta' ), 'reason'=>'page_missing' ) );
			continue;
		}
		$meta = get_post_meta( $page->ID, ALIPASANDI_SERVICE_META_KEY, true );
		$diagnosis = alipasandi_service_schema_diagnosis( $page->ID );
		$proposed = is_array( $meta ) ? $meta : array();
		$proposed['schema_version'] = ALIPASANDI_SERVICE_META_SCHEMA;
		$before = alipasandi_service_content_sha256( $meta );
		$after = alipasandi_service_content_sha256( $proposed );
		$plan['services'][ $key ] = array(
			'page_id'              => (int) $page->ID,
			'action'               => ! empty( $diagnosis['repairable'] ) ? 'set_schema_marker_only' : ( empty( $diagnosis['categories'] ) ? 'no_change' : 'blocked' ),
			'diagnosis'            => $diagnosis,
			'content_before_sha256'=> $before,
			'content_after_sha256' => $after,
			'content_unchanged'    => hash_equals( $before, $after ),
		);
	}
	$hashable = $plan;
	unset( $hashable['generated_at_utc'] );
	$plan['plan_sha256'] = hash( 'sha256', wp_json_encode( function_exists( 'alipasandi_canonicalize_snapshot' ) ? alipasandi_canonicalize_snapshot( $hashable ) : $hashable, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) );
	return $plan;
}

/** Apply an exact approved plan on staging; abort on any content-hash movement. */
function alipasandi_service_schema_repair_apply( $approved_sha256 ) {
	global $wpdb;
	if ( ! function_exists( 'wp_get_environment_type' ) || 'staging' !== wp_get_environment_type() ) {
		return new WP_Error( 'staging_only', 'Schema repair is permitted only when WP_ENVIRONMENT_TYPE=staging.' );
	}
	$plan = alipasandi_service_schema_repair_plan();
	$approved_sha256 = strtolower( trim( (string) $approved_sha256 ) );
	if ( 1 !== preg_match( '/^[a-f0-9]{64}$/', $approved_sha256 ) || ! hash_equals( $plan['plan_sha256'], $approved_sha256 ) ) {
		return new WP_Error( 'plan_hash_mismatch', 'Re-run dry-run and supply the exact current --plan-sha256.' );
	}
	$changes = array();
	$locked = array();
	$wpdb->query( 'START TRANSACTION' );
	foreach ( $plan['services'] as $key => $item ) {
		if ( 'set_schema_marker_only' !== $item['action'] || empty( $item['content_unchanged'] ) ) {
			continue;
		}
		$page_id = (int) $item['page_id'];
		$rows = $wpdb->get_results( $wpdb->prepare( "SELECT meta_id, meta_value FROM {$wpdb->postmeta} WHERE post_id = %d AND meta_key = %s FOR UPDATE", $page_id, ALIPASANDI_SERVICE_META_KEY ), ARRAY_A );
		if ( 1 !== count( $rows ) ) {
			$wpdb->query( 'ROLLBACK' );
			return new WP_Error( 'meta_row_count_changed', 'Service meta row count changed after planning: ' . $key );
		}
		$row = $rows[0];
		$before = maybe_unserialize( $row['meta_value'] );
		$before_hash = alipasandi_service_content_sha256( $before );
		if ( ! hash_equals( $item['content_before_sha256'], $before_hash ) ) {
			$wpdb->query( 'ROLLBACK' );
			return new WP_Error( 'content_changed_since_plan', 'Service content changed after the plan was generated: ' . $key );
		}
		$after = $before;
		$after['schema_version'] = ALIPASANDI_SERVICE_META_SCHEMA;
		$updated = $wpdb->update( $wpdb->postmeta, array( 'meta_value'=>maybe_serialize( $after ) ), array( 'meta_id'=>(int)$row['meta_id'] ), array( '%s' ), array( '%d' ) );
		if ( false === $updated ) {
			$wpdb->query( 'ROLLBACK' );
			return new WP_Error( 'meta_write_failed', 'Database rejected the marker-only repair: ' . $key );
		}
		$stored_raw = $wpdb->get_var( $wpdb->prepare( "SELECT meta_value FROM {$wpdb->postmeta} WHERE meta_id = %d", (int) $row['meta_id'] ) );
		$stored = maybe_unserialize( $stored_raw );
		if ( ! hash_equals( $before_hash, alipasandi_service_content_sha256( $stored ) ) ) {
			$wpdb->query( 'ROLLBACK' );
			return new WP_Error( 'content_hash_moved', 'Repair was rolled back because content changed: ' . $key );
		}
		$changes[ $key ] = array( 'page_id'=>$page_id, 'content_sha256'=>$before_hash );
		$locked[] = $page_id;
	}
	$wpdb->query( 'COMMIT' );
	foreach ( $locked as $page_id ) { clean_post_cache( $page_id ); }
	return array( 'pass'=>true, 'plan_sha256'=>$plan['plan_sha256'], 'changes'=>$changes );
}

/** Return a machine-readable health report for service and operational state. */
function alipasandi_service_health_report( $probe = false ) {
	$status = alipasandi_get_service_migration_status();
	$report = array(
		'plugin_version' => ALIPASANDI_SERVICE_CONTENT_PLUGIN_VERSION,
		'schema_version' => ALIPASANDI_SERVICE_META_SCHEMA,
		'migration_completed' => ! empty( $status['completed'] ),
		'registry_issues' => function_exists( 'alipasandi_service_registry_issues' ) ? alipasandi_service_registry_issues() : array( 'registry_unavailable' ),
		'services' => array(),
	);
	foreach ( alipasandi_service_keys() as $key ) {
		...
		if ( ! $page ) {
			...
			$diagnosis = alipasandi_service_schema_diagnosis( $page->ID );
			$issues = array_merge( $issues, $diagnosis['categories'] );
	...
	$report['pass'] = $report['migration_completed'] && empty( $report['registry_issues'] );
	...
	if ( function_exists( 'alipasandi_operational_health_report' ) ) {
		$report['operational'] = alipasandi_operational_health_report( (bool) $probe );
...
if ( defined( 'WP_CLI' ) && WP_CLI ) {
	...
	WP_CLI::add_command( 'alipasandi service health', function ( $args, $assoc_args ) {
		$report = alipasandi_service_health_report( ! empty( $assoc_args['probe'] ) );
		WP_CLI::line( wp_json_encode( $report, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE ) );
		if ( empty( $report['pass'] ) ) {
			WP_CLI::halt( 1 );
		}
	} );
	WP_CLI::add_command( 'alipasandi service schema-repair', function ( $args, $assoc_args ) {
		$plan = alipasandi_service_schema_repair_plan();
		if ( empty( $assoc_args['apply'] ) ) {
			WP_CLI::line( wp_json_encode( $plan, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) );
			return;
		}
		$result = alipasandi_service_schema_repair_apply( $assoc_args['plan-sha256'] ?? '' );
