...
/** Recursively canonicalize arrays for stable content-only comparison hashes. */
function alipasandi_service_canonicalize_value( $value ) {
	if ( ! is_array( $value ) ) { return $value; }
	$is_list = array_keys( $value ) === range( 0, count( $value ) - 1 );
	if ( ! $is_list ) { ksort( $value ); }
	foreach ( $value as $key => $item ) { $value[ $key ] = alipasandi_service_canonicalize_value( $item ); }
	return $value;
}

/** Hash service content while excluding only the schema marker. */
function alipasandi_service_payload_hash_without_schema( $meta ) {
	if ( ! is_array( $meta ) ) { return ''; }
	unset( $meta['schema_version'] );
	$meta = alipasandi_service_canonicalize_value( $meta );
	return hash( 'sha256', wp_json_encode( $meta, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) );
}

/**
 * Non-destructive plan for legacy v1 records that contain valid content but
 * are missing only the schema_version marker. Nothing is repaired silently.
 */
function alipasandi_service_schema_repair_plan() {
	$plan = array( 'schema_version'=>ALIPASANDI_SERVICE_META_SCHEMA, 'write'=>false, 'services'=>array(), 'pass'=>true );
	foreach ( alipasandi_service_keys() as $key ) {
		$page = alipasandi_get_service_page( $key );
		$entry = array( 'page_id'=>$page ? (int) $page->ID : 0, 'action'=>'blocked', 'reason'=>'', 'payload_sha256'=>'' );
		if ( ! $page ) { $entry['reason']='page_missing'; $plan['pass']=false; $plan['services'][$key]=$entry; continue; }
		if ( ! alipasandi_service_meta_exists( $page->ID ) ) { $entry['reason']='meta_missing'; $plan['pass']=false; $plan['services'][$key]=$entry; continue; }
		$meta = get_post_meta( $page->ID, ALIPASANDI_SERVICE_META_KEY, true );
		if ( ! is_array( $meta ) ) { $entry['reason']='meta_not_array'; $plan['pass']=false; $plan['services'][$key]=$entry; continue; }
		$observed = isset( $meta['schema_version'] ) ? (int) $meta['schema_version'] : 0;
		$entry['observed_schema_version'] = $observed;
		$entry['payload_sha256'] = alipasandi_service_payload_hash_without_schema( $meta );
		if ( ALIPASANDI_SERVICE_META_SCHEMA === $observed ) { $entry['action']='skip_current'; $entry['reason']='already_current'; $plan['services'][$key]=$entry; continue; }
		if ( 0 !== $observed ) { $entry['reason']='unknown_schema_version'; $plan['pass']=false; $plan['services'][$key]=$entry; continue; }

		$candidate = $meta;
		$candidate['schema_version'] = ALIPASANDI_SERVICE_META_SCHEMA;
		$sanitized = alipasandi_sanitize_service_meta( $candidate );
		if ( alipasandi_service_payload_hash_without_schema( $sanitized ) !== $entry['payload_sha256'] ) {
			$entry['reason']='sanitizer_would_change_payload';
			$plan['pass']=false;
		} else {
			$entry['action']='repair_missing_v1_marker';
			$entry['reason']='safe_content_equivalent';
		}
		$plan['services'][$key]=$entry;
	}
	$plan['plan_sha256'] = hash( 'sha256', wp_json_encode( $plan['services'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) );
	return $plan;
}

/** Apply only a fully-safe, explicitly confirmed dry-run plan. */
function alipasandi_service_schema_repair_apply( $expected_plan_sha = '' ) {
	...
	if ( 'production' === $environment ) {
		return array(
			...
			'pass' => false,
			'blocked' => true,
			'reason' => 'production_apply_forbidden_use_staging_clone_with_backup',
	...
	$result = $plan;
	$result['write'] = false;
	$expected_plan_sha = strtolower( trim( (string) $expected_plan_sha ) );
	$current_plan_sha = strtolower( (string) ( $plan['plan_sha256'] ?? '' ) );
	if ( empty( $plan['pass'] ) ) {
		$result['blocked'] = true;
		$result['reason'] = 'schema_repair_plan_not_fully_safe';
		return $result;
	}
	if ( ! preg_match( '/^[a-f0-9]{64}$/', $expected_plan_sha ) || ! hash_equals( $current_plan_sha, $expected_plan_sha ) ) {
		$result['blocked'] = true;
		$result['reason'] = 'schema_repair_apply_requires_matching_dry_run_plan_sha';
		return $result;
	}

	// Revalidate every proposed repair before the first write. Any drift stops
	// the whole operation so no partial repair begins from a stale dry-run.
	$preflight = array();
	foreach ( $plan['services'] as $key => $entry ) {
		if ( 'repair_missing_v1_marker' !== $entry['action'] ) { continue; }
		$page = alipasandi_get_service_page( $key );
		if ( ! $page ) { $result['blocked']=true; $result['reason']='prewrite_page_missing:' . $key; return $result; }
		$meta = get_post_meta( $page->ID, ALIPASANDI_SERVICE_META_KEY, true );
		if ( ! is_array( $meta ) || 0 !== (int) ( $meta['schema_version'] ?? 0 ) ) { $result['blocked']=true; $result['reason']='prewrite_state_changed:' . $key; return $result; }
		$before_hash = alipasandi_service_payload_hash_without_schema( $meta );
		if ( ! hash_equals( (string) $entry['payload_sha256'], $before_hash ) ) { $result['blocked']=true; $result['reason']='prewrite_payload_hash_drift:' . $key; return $result; }
		$candidate = $meta;
		$candidate['schema_version'] = ALIPASANDI_SERVICE_META_SCHEMA;
		$sanitized = alipasandi_sanitize_service_meta( $candidate );
		if ( ! hash_equals( $before_hash, alipasandi_service_payload_hash_without_schema( $sanitized ) ) ) { $result['blocked']=true; $result['reason']='prewrite_sanitizer_payload_drift:' . $key; return $result; }
		$preflight[ $key ] = array( 'page'=>$page, 'original'=>$meta, 'sanitized'=>$sanitized, 'payload_sha256'=>$before_hash );
	}

	$result['write'] = true;
	$written = array();
	foreach ( $preflight as $key => $prepared ) {
		$page = $prepared['page'];
		update_post_meta( $page->ID, ALIPASANDI_SERVICE_META_KEY, $prepared['sanitized'] );
		$after = get_post_meta( $page->ID, ALIPASANDI_SERVICE_META_KEY, true );
		$after_hash = alipasandi_service_payload_hash_without_schema( $after );
		if ( ! is_array( $after ) || ALIPASANDI_SERVICE_META_SCHEMA !== (int) ( $after['schema_version'] ?? 0 ) || ! hash_equals( $prepared['payload_sha256'], $after_hash ) ) {
			// Best-effort rollback of this write and every earlier repair in this run.
			...
			foreach ( array_reverse( $written, true ) as $written_key => $written_prepared ) {
				...
				$result['services'][ $written_key ]['result'] = 'rolled_back_after_verify_failure';
			}
			$result['services'][ $key ]['result']='verify_failed_rolled_back';
			$result['pass']=false;
			$result['reason']='write_verify_failed_rollback_attempted:' . $key;
			return $result;
		}
		$written[ $key ] = $prepared;
		$result['services'][$key]['result']='repaired_verified';
		$result['services'][$key]['payload_sha256_after']=$after_hash;
		alipasandi_service_log_migration( $page->ID, $key, 'schema_repair', 'missing_v1_marker_repaired_payload_hash_preserved' );
	}
	return $result;
}
...
function alipasandi_service_health_report() {
	...
	foreach ( alipasandi_service_keys() as $key ) {
		...
		$report['services'][ $key ] = array(
			...
			'observed_schema_version' => is_array( $meta ) && isset( $meta['schema_version'] ) ? (int) $meta['schema_version'] : 0,
			'payload_sha256' => is_array( $meta ) ? alipasandi_service_payload_hash_without_schema( $meta ) : '',
			'issues'  => $issues,
		);
	}
...
function alipasandi_service_tools_page() {
	...
	$schema_repair = alipasandi_service_schema_repair_plan();
	?>
	<div class="wrap" dir="rtl"><h1>Service Content</h1>
	<p>Migration فقط به‌صورت صریح اجرا می‌شود و Meta موجود را overwrite نمی‌کند.</p>
	<?php $last_schema_repair = get_transient( 'alipasandi_schema_repair_result_' . get_current_user_id() ); if ( $last_schema_repair ) { delete_transient( 'alipasandi_schema_repair_result_' . get_current_user_id() ); echo '<h2>Last schema repair result</h2><pre>' . esc_html( wp_json_encode( $last_schema_repair, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) ) . '</pre>'; } ?>
	...
	<p><strong>Schema repair:</strong> dry-run below is read-only. Apply is intentionally CLI-only and blocked on Production. Run <code>wp alipasandi service schema-repair --apply</code> only on an isolated Staging clone after DB backup + Service JSON export/hash verification.</p>
	<h2>Schema repair dry-run (non-destructive)</h2><pre><?php echo esc_html( wp_json_encode( $schema_repair, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) ); ?></pre>
...
/* Schema repair apply is intentionally WP-CLI-only; Production apply is blocked in code. */
...
if ( defined( 'WP_CLI' ) && WP_CLI ) {
	...
	WP_CLI::add_command( 'alipasandi service schema-repair', function ( $args, $assoc_args ) {
		...
		$confirm_plan_sha = isset( $assoc_args['confirm-plan-sha'] ) ? sanitize_text_field( $assoc_args['confirm-plan-sha'] ) : '';
		$result = $apply ? alipasandi_service_schema_repair_apply( $confirm_plan_sha ) : alipasandi_service_schema_repair_plan();
		WP_CLI::line( wp_json_encode( $result, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) );
		if ( empty( $result['pass'] ) ) { WP_CLI::halt( 1 ); }
	} );
