...
function alipasandi_rank_math_central_nap( $data ) {
	...
	// Runtime observation for QA/Health on contexts where a local entity is expected.
	// Store the exact Rank Math version and observation time so stale/mismatched
	// successful observations cannot survive an SEO-plugin update.
	if ( is_front_page() || is_page( 'contact' ) ) {
		$observation = array(
			'state'             => $matched_local_entity ? 'matched' : 'missing',
			'observed_at'       => time(),
			'rank_math_version' => (string) RANK_MATH_VERSION,
			'context'           => is_front_page() ? 'front_page' : 'contact',
		);
		set_transient( 'alipasandi_rank_math_local_entity_state', $observation, 30 * MINUTE_IN_SECONDS );
	}
	return $data;
}
