 ...
 */
function alipasandi_normalize_service_registry( $registry, &$issues = null, &$warnings = null ) {
	$issues = array();
	$warnings = array();
	...
	// Duplicate display labels are not an identity/security failure because form
	// values are stable keys, but they are a UX ambiguity and must be visible.
	$label_keys = array();
	foreach ( $out as $key => $item ) {
		$label_key = function_exists( 'mb_strtolower' ) ? mb_strtolower( trim( $item['label'] ), 'UTF-8' ) : strtolower( trim( $item['label'] ) );
		if ( isset( $label_keys[ $label_key ] ) ) {
			$warnings[] = 'duplicate_label:' . $label_keys[ $label_key ] . ':' . $key;
		} else {
...
function alipasandi_service_registry_state( $refresh = false ) {
	...
	if ( $refresh || null === $state ) {
		...
		$issues = array();
		$warnings = array();
		$items = alipasandi_normalize_service_registry( $raw, $issues, $warnings );
		$state = array(
			'items'    => $items,
...
function alipasandi_service_registry_warnings() {
	$state = alipasandi_service_registry_state();
	return isset( $state['warnings'] ) ? $state['warnings'] : array();
}
...
/** Stable-key => official-label map consumed by BOTH UI and server validation. */
function alipasandi_bookable_services() {
	$services = array();
	foreach ( alipasandi_service_registry() as $key => $item ) {
		if ( ! empty( $item['bookable'] ) && '' !== $item['label'] ) {
			$services[ $key ] = $item['label'];
		}
	}
	return $services;
}

/** Stable service keys accepted by the appointment handler. */
function alipasandi_allowed_services() {
	return array_keys( alipasandi_bookable_services() );
}

/** Resolve a submitted stable key to the current official Registry label. */
function alipasandi_service_label( $key ) {
	$services = alipasandi_bookable_services();
	$key = sanitize_key( $key );
	return isset( $services[ $key ] ) ? $services[ $key ] : '';
}
