	...
	<section class="inner-hero">
		<div class="site-container inner-hero-grid">
			...
			<div class="contact-info-list">
				<div class="contact-info-item"><?php echo alipasandi_icon( 'phone', 22 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><div><strong>شماره تماس</strong><?php if ( alipasandi_phone_href() ) : ?><a href="tel:<?php echo esc_attr( alipasandi_phone_href() ); ?>" dir="ltr"><?php echo esc_html( alipasandi_clinic_option( 'clinic_phone' ) ); ?></a><?php else : ?><span>نیازمند تنظیم در اطلاعات کلینیک</span><?php endif; ?></div></div>
	...
	<section class="section section-bone">
		<div class="site-container contact-grid">
			<div class="form-card" id="clinic-form" data-reveal>
				...
				<?php elseif ( 'rate_limited' === $status ) : ?><div class="form-message error" role="alert" tabindex="-1" autofocus>تعداد پیام‌های پذیرفته‌شده بیش از حد مجاز است. لطفاً ۱۵ دقیقه دیگر تلاش کنید یا مستقیماً با کلینیک تماس بگیرید.</div>
				<?php elseif ( 'invalid' === $status ) : ?><div class="form-message error" role="alert">برخی اطلاعات واردشده معتبر نیست. لطفاً نام، شماره تماس و متن پیام را بررسی کنید.</div>
				<?php elseif ( 'mail_error' === $status ) : ?><div class="form-message error" role="alert">سامانه ارسال پیام پاسخ نداد و پیام شما تأیید نشده است. لطفاً دوباره تلاش کنید یا مستقیماً با کلینیک تماس بگیرید.</div>
				<?php elseif ( in_array( $status, array( 'configuration_error', 'unavailable' ), true ) ) : ?><div class="form-message error" role="alert">فرم تماس موقتاً در دسترس نیست. لطفاً از شماره رسمی کلینیک استفاده کنید.</div>
				<?php elseif ( 'error' === $status ) : ?><div class="form-message error" role="alert">ارسال معتبر نبود. لطفاً صفحه را تازه‌سازی کرده و دوباره تلاش کنید.</div><?php endif; ?>
				<?php if ( $form_ready ) : ?>
				...
				<?php else : ?>
					<div class="form-message error" role="status" aria-live="polite" aria-atomic="true"><strong>فرم تماس موقتاً در دسترس نیست.</strong><br>لطفاً از شماره تماس رسمی کلینیک استفاده کنید.<?php if ( alipasandi_phone_href() ) : ?> <a href="tel:<?php echo esc_attr( alipasandi_phone_href() ); ?>"><span dir="ltr"><?php echo esc_html( alipasandi_clinic_option( 'clinic_phone' ) ); ?></span></a><?php endif; ?></div>
				<?php endif; ?>
			</div>
			<div class="contact-side" data-reveal>
				<div class="content-card">
					<h2>مراجعه حضوری</h2>
					<p>مطب فعال برای معاینه و درمان در <strong>نوشهر</strong> است.</p>
					<p><?php echo esc_html( alipasandi_clinic_option( 'clinic_address' ) ); ?>
					<?php if ( $maps ) : ?> — <a href="<?php echo esc_url( $maps ); ?>" target="_blank" rel="noopener noreferrer">مشاهده روی نقشه</a><?php endif; ?>
					</p>
					<p><a class="button button-gold" href="<?php echo esc_url( alipasandi_page_url( 'appointments' ) ); ?>">درخواست نوبت نوشهر</a></p>
				</div>
			</div>
		</div>
	</section>
