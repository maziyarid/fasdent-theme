# Maintenance Guide — Theme 1.4.28 / Plugin 1.3.3
...
| حوزه | Owner |
|---|---|
| Presentation/Layout | Theme 1.4.28 |
| Service storage/editor/migration/revisions/export/health | Plugin 1.3.3 |
| NAP settings + Rank Math NAP sync | Plugin 1.3.3 |
| Contact/Appointment validation, rate limit, mail request | Plugin 1.3.3 |
...
Grammar کامل در `OPENING-HOURS-CONTRACT-FA.md`. Booking slots هر خط `HH:MM`، unique/sorted، حداکثر 48. انتخاب کاربر **درخواست زمان پیشنهادی** است، نه Availability لحظه‌ای یا رزرو قطعی. Booking horizon و minimum lead-time هر کدام SSOT versioned در Plugin دارند: `alipasandi_booking_horizon_days()` و `alipasandi_booking_min_lead_minutes()`. مقدار candidate horizon فعلی 365 روز است؛ lead-time فعلی 0 دقیقه یعنی same-day مجاز است اما datetime باید strictly future باشد. هر تغییر این دو Business Rule نیازمند Owner approval + version/hash جدید است. Theme UI و Server هر دو همین policy را مصرف می‌کنند. اگر official opening hours وجود داشته باشد، روز بسته/ساعت خارج schedule رد می‌شود.
...
مرجع `SECURITY-NOTES-1.4.28-FA.md`. Raw IP ذخیره نمی‌شود؛ transient counter non-atomic است. Proxy header فقط بعد از allowlist Hosting trust شود. Abuse owner در scale = Edge/WAF.

## 10) Updates / rollback

Production pairing = Theme 1.4.28 + Plugin 1.3.3. Contract کامل: `RELEASE-ROLLBACK-CONTRACT-1.4.28-FA.md`. قبل update: DB + uploads backup، Service JSON export، hash artifacts، Rank Math config backup. Update مستقیم Production بدون Staging ممنوع.
...
## 13) Primary four-service Design Lock / Registry extensibility

Registry extensible است، اما **Primary UI عمداً Design-locked به چهار Service فعلی** (`implant`, `crown`, `surgery`, `general`) باقی می‌ماند. افزودن Registry record به‌تنهایی Home cards، Services Hub، Navigation children یا primary SEO surfaces را تغییر نمی‌دهد. Service پنجمِ primary نیازمند Theme/UI release، SEO/navigation/indexability review، medical content approval و regression QA است. Non-page bookable records فقط در Appointment flow ظاهر می‌شوند.

اگر Operational Location روزی از نوشهر تغییر کند، rule فعلی Health که `clinic_city=نوشهر` را enforce می‌کند یک config-only change نیست؛ Plugin Business Rule باید در Versioned Release تغییر کند.

## 14) Service schema marker repair

اگر Health روی migrated DB فقط `schema_invalid` با `observed_schema_version=0` نشان دهد، Tools → Service Content ابتدا **Schema repair dry-run** را بررسی کنید. Repair فقط وقتی مجاز است که sanitizer نشان دهد payload content hash بدون schema marker تغییر نمی‌کند. Apply صریح است؛ auto-repair وجود ندارد. CLI: `wp alipasandi service schema-repair` (dry-run) و فقط پس از backup/approval `wp alipasandi service schema-repair --apply=1`. Unknown schema version یا payload drift fail-closed است.
