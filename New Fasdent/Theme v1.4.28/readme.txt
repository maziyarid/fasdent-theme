...
Native RTL presentation theme. Design/Color/Layout/Grid/Typography/Hero/URL remain locked in 1.4.28.

== Production Contract ==
* Production pairing: Theme 1.4.28 + Alipasandi Service Content 1.3.3.
...
* HTML date maximum consumes Plugin 1.3.3 booking-horizon SSOT; Theme does not maintain a second horizon constant.
* No starter page is auto-created on theme activation.

== Changelog ==

= 1.4.28 =
* Pair-contract release for Plugin 1.3.3 after the WP-CLI blocker fix; no Design/URL/Layout change.
* Theme minimum companion plugin raised to 1.3.3.

= 1.4.27 =
* Same-day past-time backend validation + shared minimum lead-time policy.
* Date-aware proposal-time UI uses validated weekly opening hours; server remains authoritative.
* Appointment service submission uses stable Registry keys; labels are display/email only.
* Emergency noindex/Core-Sitemap failure policy expanded when Rank Math is absent.
* Primary four-service UI remains explicitly Design-locked.
...
* Production pairing raised to Plugin 1.3.3; no Design/URL/Layout change.
