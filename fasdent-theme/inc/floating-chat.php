<?php
/**
 * Floating contact widget (native) + Chaty compatibility.
 *
 * @package Fasdent
 */
...
function fasdent_customize_floating_chat( $wp_customize ) {
	$wp_customize->add_section(
		...
		array(
			...
			'description' => __( 'ویجت native تماس شناور. با افزونه Chaty نیز سازگار است.', 'fasdent' ),
			'priority'    => 145,
		)
	);
...
function fasdent_get_chat_channels() {
	...
	if ( $whatsapp ) {
		...
		$channels['whatsapp'] = array(
			...
			'url'   => 'https://wa.me/' . $whatsapp . ( $msg ? '?text=' . rawurlencode( $msg ) : '' ),
			'icon'  => 'fa-brands fa-whatsapp',
		);
	}
	if ( $phone ) {
		$channels['phone'] = array(
			'label' => __( 'تماس تلفنی', 'fasdent' ),
			'note'  => $phone,
			'url'   => 'tel:' . $phone,
	...
	if ( $telegram ) {
		$channels['telegram'] = array(
			'label' => __( 'تلگرام', 'fasdent' ),
			'note'  => '@' . $telegram,
			'url'   => 'https://t.me/' . rawurlencode( $telegram ),
			'icon'  => 'fa-brands fa-telegram',
		);
	}
	if ( $email ) {
		$channels['email'] = array(
			'label' => __( 'ایمیل', 'fasdent' ),
			'note'  => $email,
			'url'   => 'mailto:' . $email,
	...
	<div class="fasdent-chat fasdent-chat--<?php echo esc_attr( $position ); ?>" data-fasdent-chat>
		<div class="fasdent-chat__panel" id="<?php echo esc_attr( $panel_id ); ?>" hidden>
			<div class="fasdent-chat__header">
				<span class="fasdent-chat__avatar" aria-hidden="true"><i class="fa-solid fa-headset"></i></span>
				<div>
					<h2 class="fasdent-chat__title"><?php echo esc_html( get_theme_mod( 'fasdent_chat_title', __( 'چطور می‌توانیم کمک کنیم؟', 'fasdent' ) ) ); ?></h2>
					<p class="fasdent-chat__intro"><?php echo esc_html( get_theme_mod( 'fasdent_chat_intro', __( 'یکی از روش‌های زیر را انتخاب کنید.', 'fasdent' ) ) ); ?></p>
				</div>
				<button class="fasdent-chat__close" type="button" data-chat-close aria-label="<?php esc_attr_e( 'بستن', 'fasdent' ); ?>">
			...
			<ul class="fasdent-chat__channels">
				...
				<li>
					<a class="fasdent-chat__channel fasdent-chat__channel--<?php echo esc_attr( $key ); ?>"
					   href="<?php echo esc_url( $ch['url'] ); ?>"
					   <?php echo in_array( $key, array( 'whatsapp', 'telegram' ), true ) ? ' target="_blank" rel="noopener noreferrer"' : ''; ?>>
						<span class="fasdent-chat__channel-icon" aria-hidden="true"><i class="<?php echo esc_attr( $ch['icon'] ); ?>"></i></span>
						<span>
							<span class="fasdent-chat__channel-label"><?php echo esc_html( $ch['label'] ); ?></span>
							<span class="fasdent-chat__channel-note"><?php echo esc_html( $ch['note'] ); ?></span>
						</span>
		...
		<button class="fasdent-chat__launcher" type="button" data-chat-toggle
				aria-expanded="false" aria-controls="<?php echo esc_attr( $panel_id ); ?>">
			<span class="fasdent-chat__launcher-icon" aria-hidden="true"><i class="fa-solid fa-comments"></i></span>
