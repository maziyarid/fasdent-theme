...
function alipasandi_rank_math_central_nap( $data ) {
	...
	foreach ( $data as $index => $entity ) {
		...
		$data[ $index ]['name'] = 'کلینیک دندانپزشکی دکتر کیوان علی‌پسندی';
		$data[ $index ]['url'] = home_url( '/' );
		$data[ $index ]['telephone'] = alipasandi_phone_href();
		$data[ $index ]['hasMap'] = alipasandi_clinic_option( 'clinic_maps' );
		$data[ $index ]['address'] = array( '@type'=>'PostalAddress', 'streetAddress'=>alipasandi_clinic_option( 'clinic_street' ), 'addressLocality'=>alipasandi_clinic_option( 'clinic_city' ), 'addressRegion'=>'مازندران', 'addressCountry'=>'IR' );
