...
define( 'ALIPASANDI_THEME_VERSION', '1.4.23' );
define( 'ALIPASANDI_SERVICE_PLUGIN_MIN_VERSION', '1.1.0' );
...
/* Compatibility NAP layer only when the site plugin is unavailable. */
...
function alipasandi_service_plugin_state() {
	...
	return version_compare( ALIPASANDI_SERVICE_CONTENT_PLUGIN_VERSION, ALIPASANDI_SERVICE_PLUGIN_MIN_VERSION, '>=' ) ? 'compatible' : 'outdated';
}

function alipasandi_service_plugin_notice() {
	$state = alipasandi_service_plugin_state();
	if ( 'compatible' === $state || ! current_user_can( 'activate_plugins' ) ) {
		return;
	}
	$message = 'missing' === $state
		? 'افزونه Alipasandi Service Content غیرفعال/حذف شده است. Frontend از Meta موجود استفاده می‌کند، اما مدیریت NAP، فرم‌ها و Service Health در حالت compatibility است.'
		: 'نسخه Alipasandi Service Content قدیمی است. Theme 1.4.23 به Plugin 1.1.0 یا جدیدتر نیاز دارد.';
	echo '<div class="notice notice-error"><p><strong>' . esc_html( $message ) . '</strong></p></div>';
}
add_action( 'admin_notices', 'alipasandi_service_plugin_notice' );

function alipasandi_theme_site_health_tests( $tests ) {
	$tests['direct']['alipasandi_service_plugin'] = array(
		'label' => 'Alipasandi Service Plugin compatibility',
		'test'  => function () {
			$state = alipasandi_service_plugin_state();
			return array(
				'label'       => 'compatible' === $state ? 'افزونه عملیاتی سازگار است' : 'افزونه عملیاتی Missing/Outdated است',
				'status'      => 'compatible' === $state ? 'good' : 'critical',
				'badge'       => array( 'label' => 'Alipasandi', 'color' => 'blue' ),
				'description' => '<p>' . esc_html( 'Theme 1.4.23 requires Alipasandi Service Content >= 1.1.0. Current state: ' . $state ) . '</p>',
				'test'        => 'alipasandi_service_plugin',
			);
		},
	);
	return $tests;
}
add_filter( 'site_status_tests', 'alipasandi_theme_site_health_tests' );
