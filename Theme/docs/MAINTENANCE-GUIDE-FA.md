...
- **SEO Owner Production:** فقط Rank Math. تغییر SEO plugin نیازمند Release و Migration رسمی جدید است.
...
- **Host normalization Owner:** Server/Hosting. **Page-level Redirect Owner:** Rank Math.
...
روش Restore مرحله‌ای: (۱) Maintenance/Backup window، (۲) Restore دیتابیس، (۳) Restore کامل `wp-content/uploads`، (۴) نصب Artifactهای Theme/Plugin با SHA مطابق Manifest، (۵) Import تنظیمات Rank Math از Backup همان Release، (۶) اجرای `wp alipasandi service health`، (۷) Purge همه Page/Object/CDN cache، (۸) Smoke فرم، Media، NAP، Sitemap و صفحات Service. Domain migration فقط با `wp search-replace --all-tables-with-prefix --precise` یا ابزار serialization-safe انجام شود؛ raw SQL ممنوع است.
...
Registry در Plugin با فیلتر `alipasandi_service_registry` توسعه‌پذیر است، اما Release فعلی فقط چهار Key را ثبت می‌کند و هیچ Service پنجم در UI ندارد. افزودن Service جدید نیازمند Release کد کنترل‌شده برای افزودن Key، Page/Template mapping، Migration/Health، Navigation، SEO و QA است؛ صرف ساخت Page کافی نیست.
...
| Plugin Site Functionality | `Alipasandi Service Content 1.2.0` |
