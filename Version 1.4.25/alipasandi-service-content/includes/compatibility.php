...
define( 'ALIPASANDI_SERVICE_PRODUCTION_THEME_MIN', '1.4.25' );
...
function alipasandi_plugin_compatibility_notice() {
	...
	if ( 'theme_outdated' === $state ) {
		echo '<div class="notice notice-error"><p><strong>' . esc_html__( 'Alipasandi Service Content 1.3.0 requires Alipasandi Clinic theme 1.4.24 or newer to load safely. Production pairing requires theme 1.4.25.', 'alipasandi-service-content' ) . '</strong></p></div>';
	} elseif ( 'compatibility_only' === $state ) {
		echo '<div class="notice notice-warning"><p><strong>' . esc_html__( 'Theme 1.4.24 is a superseded compatibility build. Use Theme 1.4.25 with Plugin 1.3.0 for the production-candidate contract.', 'alipasandi-service-content' ) . '</strong></p></div>';
	}
}
