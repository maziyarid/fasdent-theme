...
/** Calendar-valid, WordPress-timezone-aware, max 365-day appointment horizon. */
function alipasandi_validate_appointment_date( $date ) {
	...
	$max = $today->modify( '+365 days' );
	if ( $dt < $today ) { return new WP_Error( 'past_date', 'تاریخ گذشته پذیرفته نمی‌شود.' ); }
	if ( $dt > $max ) { return new WP_Error( 'date_too_far', 'تاریخ درخواستی بیش از 365 روز آینده است.' ); }
	return $dt;
}
