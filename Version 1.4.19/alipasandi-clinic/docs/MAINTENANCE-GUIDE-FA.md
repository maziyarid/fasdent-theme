...
## ۷. صفحات خدمات (Service Content) — از ۱.۴.۱۹

**اولویت منبع داده:**  
`Post Meta (_alipasandi_service)` → در صورت خالی بودن → `inc/service-data.php` (فقط خواندنی / Fallback)

| مورد | جزئیات |
|---|---|
| محل ویرایش | ویرایش صفحه خدمت در پیشخوان → Meta Box «محتوای صفحه خدمت» |
| صفحات | Implant · Crown · Surgery · General (فقط همین‌ها Meta Box می‌بینند) |
| Meta key | `_alipasandi_service` (در `wp_postmeta` — با Backup دیتابیس منتقل می‌شود) |
| Migration | یک‌باره و Idempotent؛ اگر Meta قبلاً پر باشد، Update Theme آن را overwrite نمی‌کند |
| H1 | یک تگ `<h1>` واحد؛ `title` + `title_gold` فقط ظاهر (span.gold-text) |
| FAQ | از Meta Box قابل ویرایش؛ Accordion همچنان HTML قابل Crawl + ARIA |
| Layout/Design | بدون تغییر — Template کنترل می‌کند |

فیلدها شامل: Hero (eyebrow, title, title_gold, intro, image, ALT)، بخش چیست، مزایا، مراحل، ارزیابی، FAQ، CTA.
