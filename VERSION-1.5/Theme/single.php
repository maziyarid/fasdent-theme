...
<main id="main-content" class="content-area">
	<?php while ( have_posts() ) : the_post(); ?>
		<?php $categories = wp_get_post_categories( get_the_ID(), array( 'fields' => 'names' ) ); ?>
			<?php ob_start(); the_title(); $title_html = ob_get_clean(); ob_start(); ?><div class="entry-meta"><span>نویسنده: <?php the_author(); ?></span><span>انتشار: <?php echo esc_html( get_the_date() ); ?></span><?php if ( get_the_modified_time( 'U' ) > get_the_time( 'U' ) ) : ?><span>آخرین بازبینی: <?php echo esc_html( get_the_modified_date() ); ?></span><?php endif; ?></div><?php $meta_html = ob_get_clean(); alipasandi_inner_hero( 'header', esc_html( $categories ? implode( '، ', $categories ) : 'مقاله آموزشی' ), $title_html, '', $meta_html ); ?>
