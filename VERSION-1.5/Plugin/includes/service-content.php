...
function alipasandi_service_keys() {
	$registry = apply_filters( 'alipasandi_service_registry', array( 'implant', 'crown', 'surgery', 'general' ) );
	$registry = is_array( $registry ) ? array_map( 'sanitize_key', $registry ) : array();
	return array_values( array_unique( array_filter( $registry ) ) );
}
